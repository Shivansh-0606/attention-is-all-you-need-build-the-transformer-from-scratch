...
def select_top_k_candidates(candidate_scores, k):
    ...
    top_scores , flat_indices = torch.topk(flat_scores , k , largest = True , sorted = True)

    beam_indices = torch.div(flat_indices , vocab_size , rounding_mode = 'floor').to(dtype = torch.int64)

    token_ids = (flat_indices % vocab_size).to(dtype = torch.int64)
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    parent_seq = beam_sequences.index_select(0 , beam_indices)
    new_col = token_ids.unsqueeze(-1)

    return torch.cat([parent_seq , new_col] , dim = -1).to(dtype = torch.int64)
